...
export async function runBenchmark(name: string, transform: TransformModule, inputPath: string, renderOptions?: RenderOptions): Promise<BenchmarkResult> {
	...
	try {
		...
		if (transformTiming) {
			return {
				name,
				totalMs: transformTiming.timing.totalMs,
				samplesPerSecond: transformTiming.timing.samplesPerSecond,
				realTimeMultiplier: transformTiming.timing.realTimeMultiplier,
			};
		}

		const renderTiming = (source as SourceModule).renderTiming;
