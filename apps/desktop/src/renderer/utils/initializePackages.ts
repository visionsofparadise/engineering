...
function updatePackage(
	...
	index: number,
	update: Partial<ModulePackageState>,
): void {
...
export async function initializePackage(
	...
	try {
		...
		try {
			...
			// no package.json or no version
		}

		updatePackage(appStore, app, index, { status: "ready", modules: [...modules], version });
	} catch (error) {
		const message = error instanceof Error ? error.message : String(error);
		updatePackage(appStore, app, index, { status: "error", error: message });
	}
}
