...
```bash
npm install audio-chain-module
```
...
```ts
import { chain, read, normalize, write } from "audio-chain-module";
...
```ts
import { chain, read, fan, normalize, trim, write } from "audio-chain-module";
...
```bash
npx engineering-acm process --pipeline pipeline.ts
```

```ts
// pipeline.ts
import { chain, read, normalize, trim, write } from "audio-chain-module";
