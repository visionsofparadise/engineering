...
export class LoudnessModule extends FfmpegModule {
	...
	override async _process(buffer: ChunkBuffer): Promise<void> {
		...
		this.measuredValues = await measureLoudness(buffer, context, this.properties);
...
async function measureLoudness(
	...
	const binaryPath = await resolveBinary("ffmpeg", properties.binaryPath);
...
export function loudness(options?: { target?: number; truePeak?: number; lra?: number; binaryPath?: string; id?: string }): LoudnessModule {
	return new LoudnessModule({
